...
#[Cli(
    command: 'dev:blueprint:publish',
    description: 'Publish blueprint registry changes to remote repository',
    usage: 'dev:blueprint:publish',
    examples: [
        'dev:blueprint:publish',
    ]
)]
final class BlueprintPublishCommand extends Command
{
    use CliGenerator;
    ...
    public function execute(array $args): int
    {
        ...
        $config = $this->registryService->getRegistryConfig('blueprint');

        if (!$config) {
            $this->info('Auto-detecting blueprint registry configuration from git repository...');
            $config = $this->registryService->getRegistryConfigOrDetect('blueprint');
        }
        ...
        if (!isset($config['url']) || $config['url'] === null) {
            ...
            $this->info('Run: php forge.php dev:blueprint:init --url=<repository_url>');
            return 1;
        }
        ...
        $this->success("Blueprint registry published successfully!");
        return 0;
    }
}
