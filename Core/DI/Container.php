    ...
    private function loadModuleByClass(string $class): void
    {
        ...
        if (!empty($matches[1])) {
            ...
            try {
                ...
                Logger::log("Loader service not available for namespace '{$namespacePrefix}'", $e->getMessage());
            } catch (\Throwable $e) {
                Logger::log("Error loading module for namespace '{$namespacePrefix}'", $e->getMessage());
            }
        }
    }
    ...
    public function getAll(string $interface): array
    {
        ...
        if (isset($this->interfaceMapCompiled[$interface])) {
            ...
            foreach ($this->interfaceMapCompiled[$interface] as $serviceId) {
                try {
                    ...
                    Logger::log("getAll(): failed to resolve service '{$serviceId}' for interface '{$interface}'", $e->getMessage());
                }
            }
        ...
        if (isset($this->services[$interface])) {
            try {
                ...
                Logger::log("getAll(): failed to get service '{$interface}'", $e->getMessage());
            }
        }
                ...
                try {
                    ...
                    if ($reflectionClass->implementsInterface($interface)) {
                        ...
                        try {
                            ...
                            Logger::log("getAll(): failed to instantiate service '{$serviceId}' for interface '{$interface}'", $e->getMessage());
                        }
                    }
